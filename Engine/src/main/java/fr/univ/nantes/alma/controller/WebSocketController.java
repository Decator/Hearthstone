...
public class WebSocketController {
	...
	public void endTurn(String uuidGame) {
		this.template.convertAndSend("/game/" + uuidGame, Application.engineBridge.endTurn(UUID.fromString(uuidGame)));
	}
	...
	public void playCard(String data) {
		...
		try {
			this.template.convertAndSend("/game/" + dataSplit[0],
					Application.engineBridge.playCard(UUID.fromString(dataSplit[0]), Integer.parseInt(dataSplit[1]),
							UUID.fromString(dataSplit[2]), Integer.parseInt(dataSplit[3])));
	...
	public void attack(String data) {
		...
		try {
			this.template.convertAndSend("/game/" + dataSplit[0], Application.engineBridge.attack(
					UUID.fromString(dataSplit[0]), Integer.parseInt(dataSplit[1]), Integer.parseInt(dataSplit[2])));
		} catch (EngineException e) {
			this.template.convertAndSend("/game/" + dataSplit[0], e.getMessage());
		}
	}
