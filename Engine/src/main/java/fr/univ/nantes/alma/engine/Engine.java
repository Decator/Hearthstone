...
public class Engine implements EngineBridge {
  ...
  public GameMethods endTurn(UUID uuidGame) {
    this.games.get(uuidGame).endTurn();
  ...
  public GameMethods playCard(UUID uuidGame, int idCard, UUID uuidPlayer, int idTarget) 
    ...
    try {
      if (this.games.get(uuidGame).getOtherPlayer().getUuid().equals(uuidPlayer)) {
        this.games.get(uuidGame).playCard(idCard, this.games.get(uuidGame).getOtherPlayer(), 
            idTarget);
      } else {
        this.games.get(uuidGame).playCard(idCard, this.games.get(uuidGame).getCurrentPlayer(), 
            idTarget);
      }
  ...
  @Override
  public GameMethods heroPower(UUID uuidGame, UUID uuidPlayer, int idTarget) 
      throws EngineException {
    try {
      if (this.games.get(uuidGame).getCurrentPlayer().getUuid().equals(uuidPlayer)) {
        this.games.get(uuidGame).heroPower(this.games.get(uuidGame).getCurrentPlayer(), idTarget);
      } else {
        this.games.get(uuidGame).heroPower(this.games.get(uuidGame).getOtherPlayer(), idTarget);
      }
    } catch (EngineException e) {
      throw e;
    }
  ...
  public GameMethods attack(UUID uuidGame, int idAttack, int idTarget) throws EngineException {
    try {
      this.games.get(uuidGame).attack(idAttack, idTarget);
    } catch (EngineException e) {
      throw e;
    }
    return this.games.get(uuidGame);
