	...
	public GameMethods playCard(UUID uuidGame, int idCard, UUID uuidPlayer, int idTarget) throws EngineException {
		try {
			if (this.games.get(uuidGame).getOtherPlayer().getUuid().equals(uuidPlayer)) {
				this.games.get(uuidGame).playCard(idCard, this.games.get(uuidGame).getOtherPlayer(), idTarget);
			} else {
				this.games.get(uuidGame).playCard(idCard, this.games.get(uuidGame).getCurrentPlayer(), idTarget);
			}
